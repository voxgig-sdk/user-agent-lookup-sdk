...
import (
	...
	omni "github.com/voxgig-sdk/user-agent-lookup-sdk/go/test/omni"
...
func omniProvider(client any) *omni.Provider {
	provider := &omni.Provider{
		...
		Errify: func(err any) any {
			if sdkerr, is := err.(*sdk.UserAgentLookupError); is {
				out := map[string]any{
					"name":    "UserAgentLookupError",
...
func omniCtx(arg any, client *sdk.UserAgentLookupSDK, utility *sdk.Utility) *sdk.Context {
	...
	if provider, is := ctxmap["client"].(*omni.Provider); is {
		if live, is := defproviderclient(provider).(*sdk.UserAgentLookupSDK); is {
			client = live
