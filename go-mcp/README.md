# user-agent-lookup-mcp

[MCP](https://modelcontextprotocol.io) server exposing the UserAgentLookup SDK as
two agent tools — `user-agent-lookup_list` and `user-agent-lookup_load` — built on the
...
```sh
# 1. Build a native binary (-> dist/<os>-<arch>/user-agent-lookup-mcp)
make build

# 2. Provide credentials via the environment
export USER_AGENT_LOOKUP_APIKEY=sk_live_xxx

# 3a. Install into Claude Code over stdio (most common)
claude mcp add --scope user user-agent-lookup \
  -- /absolute/path/to/user-agent-lookup-mcp -transport stdio

# 3b. …or run a shared HTTP server instead
./user-agent-lookup-mcp -transport http -addr :8080
```

Tool-call arguments (what an agent sends):

```jsonc
// user-agent-lookup_list: first page of records
{ "entity": "user_agent" }
{ "entity": "user_agent", "query": { } }

// user-agent-lookup_load: one record by id
{ "entity": "user_agent", "query": { "id": 1 } }
```
...
1. **Build** the server from this `go-mcp/` directory:
   ...
   ```sh
   make build          # -> dist/<os>-<arch>/user-agent-lookup-mcp
   ```
...
3. **Install it into Claude Code** (stdio transport):
   ...
   ```sh
   claude mcp add --scope user user-agent-lookup \
     -- "$PWD"/dist/*/user-agent-lookup-mcp -transport stdio
   ```

4. **Restart Claude Code.** The `user-agent-lookup_list` and `user-agent-lookup_load` tools now appear
   in new sessions. Ask the agent to *"list user_agent using user-agent-lookup"*
   and it calls `user-agent-lookup_list` with `{"entity":"user_agent"}`.
...
```sh
export USER_AGENT_LOOKUP_APIKEY=sk_live_xxx            # API key
export USER_AGENT_LOOKUP_BASE=https://api.example.com  # optional: override the API base URL
```
...
### Call the `user-agent-lookup_list` tool
...
### Call the `user-agent-lookup_load` tool
...
| Tool | Args | Returns |
|------|------|---------|
| `user-agent-lookup_list` | `entity` (required), `query` (optional map) | First page of records as JSON |
| `user-agent-lookup_load` | `entity` (required), `query` = `{id:N}` | Single record as JSON |
...
| Variable | Purpose |
|----------|---------|
| `USER_AGENT_LOOKUP_APIKEY` | API key sent with every request. |
| `USER_AGENT_LOOKUP_BASE` | Optional override of the API base URL. |
...
```sh
./user-agent-lookup-mcp -transport http -addr :18080 &
...
curl -sN -X POST http://localhost:18080 \
  ...
  -d '{"jsonrpc":"2.0","id":2,"method":"tools/call","params":{"name":"user-agent-lookup_load","arguments":{"entity":"user_agent","query":{"id":1}}}}'
```
