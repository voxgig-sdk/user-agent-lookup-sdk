// Command UserAgentLookup-cli is a CLI + REPL for the UserAgentLookup SDK,
...
const prompt = "user-agent-lookup"

// entitiesHelp is the space-separated entity list shown by /help.
const entitiesHelp = "user_agent"
...
func usage(out io.Writer) {
	...
	fmt.Fprintln(out, "Env:      USER_AGENT_LOOKUP_APIKEY (api key), USER_AGENT_LOOKUP_BASE (base url override)")
...
func run(args []string, in io.Reader, out, errOut io.Writer) int {
	...
	// Configure from the environment: USER_AGENT_LOOKUP_APIKEY carries the API key and
	// USER_AGENT_LOOKUP_BASE optionally overrides the API base URL (e.g. production).
	// Both injectable by a secrets vault. Unset -> nil config defaults.
	var opts map[string]any
	if apikey := os.Getenv("USER_AGENT_LOOKUP_APIKEY"); apikey != "" {
		opts = map[string]any{"apikey": apikey}
	}
	if base := os.Getenv("USER_AGENT_LOOKUP_BASE"); base != "" {
		if opts == nil {
			opts = map[string]any{}
		}
		opts["base"] = base
	}
	client := sdk.NewUserAgentLookupSDK(opts)
