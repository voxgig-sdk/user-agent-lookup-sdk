...
boru-driven command-line client **and** interactive REPL for the UserAgentLookup
...
```sh
# 1. Build a native binary (-> dist/<os>-<arch>/user-agent-lookup-cli)
make build

# 2. See usage (words, entities, env vars)
./user-agent-lookup-cli --help

# 3. Provide credentials once, via the environment
export USER_AGENT_LOOKUP_APIKEY=sk_live_xxx

# 4. Each command line is ONE boru expression, run against the API:
./user-agent-lookup-cli load 1 user_agent            # {id:1} shorthand
./user-agent-lookup-cli load '{id:1}' user_agent       # explicit match map

# 5. Override the API base URL for a single call
USER_AGENT_LOOKUP_BASE=https://api.example.com ./user-agent-lookup-cli load 1 user_agent

# 6. No arguments -> interactive REPL
./user-agent-lookup-cli
user-agent-lookup> load 1 user_agent
user-agent-lookup> /quit
```
...
1. **Build the binary.** From this `go-cli/` directory:
   ...
   ```sh
   make build          # -> dist/<os>-<arch>/user-agent-lookup-cli
   ```
...
3. **Run a query.** Evaluate an boru expression against the API (or run with no
   ...
   ```sh
   ./dist/*/user-agent-lookup-cli load 1 user_agent
   ```
...
```sh
./user-agent-lookup-cli load 1 user_agent          # scalar shorthand for {id:1}
./user-agent-lookup-cli load '{id:1}' user_agent     # explicit match map
```
...
Run with no arguments to open a REPL (prompt `user-agent-lookup>`). Each line is
evaluated as its own boru expression:

```text
$ ./user-agent-lookup-cli
user-agent-lookup> load 1 user_agent
user-agent-lookup> /help
user-agent-lookup> /quit
```
...
- `<entity>` is a bareword, auto-quoted as an boru atom (e.g. `user_agent`).
...
| Target | Result |
|--------|--------|
| `make build` | Native binary at `dist/<os>-<arch>/user-agent-lookup-cli`. |
...
(`./user-agent-lookup-cli <expr>`) and interactively (the REPL), and expressions compose the
