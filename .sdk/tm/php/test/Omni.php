...
//    only - a typed UserAgentLookupContext there would make every ctx
...
final class UserAgentLookupOmni
{
    ...
    public static function ctx_from_map($ctxmap, $client, $utility)
    {
        ...
        $ctx = new UserAgentLookupContext($clean, null);
        ...
        if (isset($clean['spec']) && is_array($clean['spec'])) {
            $ctx->spec = new UserAgentLookupSpec($clean['spec']);
        }

        if (isset($clean['result']) && is_array($clean['result'])) {
            $ctx->result = new UserAgentLookupResult($clean['result']);
            if (isset($clean['result']['err']) && is_array($clean['result']['err'])) {
                $msg = $clean['result']['err']['message'] ?? '';
                $ctx->result->err = new UserAgentLookupError('', is_string($msg) ? $msg : '');
            }
        }

        if (isset($clean['response']) && is_array($clean['response'])) {
            $ctx->response = new UserAgentLookupResponse($clean['response']);
    ...
    public static function err_from_map($m): ?UserAgentLookupError
    {
        if (!is_array($m)) {
            return null;
        }
        ...
        $code = $m['code'] ?? '';
        return new UserAgentLookupError(is_string($code) ? $code : '', $msg);
    }
