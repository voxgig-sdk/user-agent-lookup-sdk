...
// UserAgentLookup SDK feature test
//
...
class FtCtrl extends UserAgentLookupControl
{
...
class FtClient
{
    ...
    private UserAgentLookupUtility $utility;

    public function __construct(UserAgentLookupUtility $utility, array $options)
    {
    ...
    public function get_utility(): UserAgentLookupUtility
    {
        return $this->utility;
    }
}
...
class FtRecorder
{
    ...
    {
        return function (UserAgentLookupContext $_ctx, string $url, array $fetchdef): array {
...
class FtHarness
{
    ...
    public UserAgentLookupContext $rootctx;
    ...
    public function __construct(
        ...
        $utility->fetcher = $server ?? function (UserAgentLookupContext $_ctx, string $_url, array $fetchdef): array {
        ...
        foreach ($features as $fspec) {
            ...
            $f = UserAgentLookupConfig::make_feature($name);
    ...
    private static function buildUrl(UserAgentLookupSpec $spec): string
    {
        $keys = [];
    ...
    public function op(array $o = []): array
    {
        ...
        if (($ctx->out['point'] ?? null) instanceof UserAgentLookupError) {
        ...
        if (is_array($fetched)) {
            $ctx->response = new UserAgentLookupResponse($fetched);
        }
    ...
    private function fail(UserAgentLookupContext $ctx, mixed $err): array
    {
...
class FeatureTest extends TestCase
{
    ...
    public function test_retry_retries_a_transport_error_until_exhausted(): void
    {
        ...
        $clock = new FtClock();
        $n = 0;
        $server = function (UserAgentLookupContext $ctx, string $_url, array $_fd) use (&$n): array {
            $n++;
