# UserAgentLookup SDK test runner

from __future__ import annotations
import os
import json

from useragentlookup_sdk.utility.voxgig_struct import voxgig_struct as vs


class UserAgentLookupTestRunner:
    _env = {}
    ...
    def load_env_local():
        ...
        for line in content.splitlines():
            ...
            UserAgentLookupTestRunner._env[key] = val

    @staticmethod
    def getenv(key):
        val = UserAgentLookupTestRunner._env.get(key)
        if val is not None:
            return val
    ...
    @staticmethod
    def env_override(m):
        live = UserAgentLookupTestRunner.getenv("USER_AGENT_LOOKUP_TEST_LIVE")
        override = UserAgentLookupTestRunner.getenv("USER_AGENT_LOOKUP_TEST_OVERRIDE")

        if live == "TRUE" or override == "TRUE":
            for key in list(m.keys()):
                envval = UserAgentLookupTestRunner.getenv(key)
        ...
        explain = UserAgentLookupTestRunner.getenv("USER_AGENT_LOOKUP_TEST_EXPLAIN")
    ...
    def load_test_control():
        ...
        """
        if UserAgentLookupTestRunner._test_control is not None:
            return UserAgentLookupTestRunner._test_control
        ctrl_path = os.path.join(os.path.dirname(__file__), "sdk-test-control.json")
        try:
            with open(ctrl_path, "r") as f:
                UserAgentLookupTestRunner._test_control = json.load(f)
        except (FileNotFoundError, IOError, ValueError):
            UserAgentLookupTestRunner._test_control = {
                "version": 1,
                "test": {"skip": {
    ...
    def is_control_skipped(kind, name, mode):
        ...
        ctrl = UserAgentLookupTestRunner.load_test_control()
...
def load_env_local():
    UserAgentLookupTestRunner.load_env_local()


def env_override(m):
    return UserAgentLookupTestRunner.env_override(m)


def entity_data(v):
    return UserAgentLookupTestRunner.entity_data(v)


def entity_list_to_data(lst):
    return UserAgentLookupTestRunner.entity_list_to_data(lst)


def is_control_skipped(kind, name, mode):
    return UserAgentLookupTestRunner.is_control_skipped(kind, name, mode)


def load_test_control():
    return UserAgentLookupTestRunner.load_test_control()


def live_client_options():
    return UserAgentLookupTestRunner.live_client_options()


def live_delay_ms():
    return UserAgentLookupTestRunner.live_delay_ms()
