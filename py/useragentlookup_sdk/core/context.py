...
from useragentlookup_sdk.core.helpers import get_ctx_prop, to_map
...
class UserAgentLookupContext:
    def __init__(self, ctxmap=None, basectx=None):
        ...
        # Ctrl
        self.ctrl = UserAgentLookupControl()
        ...
        if isinstance(sp, UserAgentLookupSpec):
            self.spec = sp
        elif basectx is not None:
        ...
        if isinstance(r, UserAgentLookupResult):
            self.result = r
        elif basectx is not None:
        ...
        if isinstance(rp, UserAgentLookupResponse):
            self.response = rp
        elif basectx is not None:
    ...
    def resolve_op(self, opname):
        ...
        if opname == "":
            return UserAgentLookupOperation({})
    ...
    def make_error(self, code, msg):
        return UserAgentLookupError(code, msg, self)
