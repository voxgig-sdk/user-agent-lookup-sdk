# UserAgentLookup TypeScript SDK



The TypeScript SDK for the UserAgentLookup API — a type-safe, entity-oriented client with full async/await support.
...
```ts
try {
  const useragent = await client.UserAgent().load({ id: 'example_id' })
...
```ts
new UserAgentLookupSDK(options?: {
  base?: string
  prefix?: string
  suffix?: string
...
| `tester(testopts?, sdkopts?)` | `UserAgentLookupSDK` | Create a test-mode client instance. |

#### Static methods

| Method | Returns | Description |
| --- | --- | --- |
| `UserAgentLookupSDK.test(testopts?, sdkopts?)` | `UserAgentLookupSDK` | Create a test-mode client. |
...
| `client` | `client(): UserAgentLookupSDK` | Return the parent SDK client. |
...
user-agent-lookup/
├── src/
│   ├── UserAgentLookupSDK.ts        # Main SDK class
...
await useragent.load({ id: "example_id" })

// useragent.data() now returns the loaded useragent data
// useragent.match() returns { id: "example_id" }
```
