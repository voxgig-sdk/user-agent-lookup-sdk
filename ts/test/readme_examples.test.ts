...
//     (\`new UserAgentLookupSDK(...)\` / \`UserAgentLookupSDK.test(...)\` become
//     \`UserAgentLookupSDK.test({ entity: ... })\`), transpiled to JS, and run
...
const SDK_NAME = 'UserAgentLookupSDK'
...
const TEST_SEED = {"entity":{"user_agent":{"test01":{"id":"test01"}}}}
...
// rewrite real client construction (\`new UserAgentLookupSDK(...)\` and any
// \`UserAgentLookupSDK.test(...)\`) to a seeded test-mode client, and give
...
function rewriteForRun(code: string): string {
  ...
  out = out.replace(/new\s+UserAgentLookupSDK\s*\([^)]*\)/g, () => SEEDED_CTOR)
  out = out.replace(/UserAgentLookupSDK\.test\s*\([^)]*\)/g, () => SEEDED_CTOR)
