...
class UserAgentEntityTest < Minitest::Test
  ...
  def test_basic_flow
    setup = user_agent_basic_setup(nil)
    # Per-op sdk-test-control.json skip.
    _live = setup[:live] || false
    ["load"].each do |_op|
      _should_skip, _reason = Runner.is_control_skipped("entityOp", "user_agent." + _op, _live ? "live" : "unit")
      if _should_skip
    ...
    if setup[:synthetic_only]
      skip "live entity test uses synthetic IDs from fixture — set USER_AGENT_LOOKUP_TEST_USER_AGENT_ENTID JSON to run live"
      return
    end
    client = setup[:client]
    ...
    user_agent_ref01_data = nil
    if user_agent_ref01_data_raw.length > 0
...
def user_agent_basic_setup(extra)
  Runner.load_env_local

  entity_data_file = File.join(__dir__, "..", "..", ".sdk", "test", "entity", "user_agent", "UserAgentTestData.json")
  ...
  entid_env_raw = ENV["USER_AGENT_LOOKUP_TEST_USER_AGENT_ENTID"]
  ...
  if env["USER_AGENT_LOOKUP_TEST_LIVE"] == "TRUE"
    ...
    client = UserAgentLookupSDK.new(Helpers.to_map(merged_opts))
  end
