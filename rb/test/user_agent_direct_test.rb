...
class UserAgentDirectTest < Minitest::Test
  def test_direct_load_user_agent
    setup = user_agent_direct_setup({ "id" => "direct01" })
    _should_skip, _reason = Runner.is_control_skipped("direct", "direct-load-user_agent", setup[:live] ? "live" : "unit")
    if _should_skip
...
def user_agent_direct_setup(mockres)
