...
require_relative 'UserAgentLookup_types'
...
class UserAgentLookupSDK
  ...
  def initialize(options = {})
    ...
    feature_opts = UserAgentLookupHelpers.to_map(VoxgigStruct.getprop(@options, "feature"))
    if feature_opts
      featureorder = VoxgigStruct.getpath(@options, "__derived__.featureorder")
      if featureorder.is_a?(Array)
        featureorder.each do |fname|
          fopts = UserAgentLookupHelpers.to_map(feature_opts[fname])
          if fopts && fopts["active"] == true
            utility.feature_add.call(@_rootctx, UserAgentLookupFeatures.make_feature(fname))
          end
        end
      end
    end
  ...
  def get_utility
    UserAgentLookupUtility.copy(@_utility)
  end
  ...
  def prepare(fetchargs = {})
    ...
    ctrl = UserAgentLookupHelpers.to_map(VoxgigStruct.getprop(fetchargs, "ctrl")) || {}
    ...
    params = UserAgentLookupHelpers.to_map(VoxgigStruct.getprop(fetchargs, "params")) || {}
    query = UserAgentLookupHelpers.to_map(VoxgigStruct.getprop(fetchargs, "query")) || {}
  ...
  def raw_request(fetchargs = {})
    ...
    rescue UserAgentLookupError => err
    ...
    if fetched.is_a?(Hash)
      status = UserAgentLookupHelpers.to_int(VoxgigStruct.getprop(fetched, "status"))
  ...
  def graphql(query, variables = nil, ctrl = nil)
    ...
    if errors.is_a?(Array) && !errors.empty?
      ...
      res["ok"] = false
      res["err"] = UserAgentLookupError.new(
        "graphql_error", "UserAgentLookupSDK: graphql: #{msg}")
      res["graphql"] = errors
    end
  ...
  def self.test(testopts = nil, sdkopts = nil)
    ...
    sdk = UserAgentLookupSDK.new(sdkopts)
    sdk.mode = "test"
    sdk
  end
end
