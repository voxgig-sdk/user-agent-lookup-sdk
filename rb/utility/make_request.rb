...
module UserAgentLookupUtilities
  MakeRequest = ->(ctx) {
    ...
    spec = ctx.spec
    utility = ctx.utility
    response = UserAgentLookupResponse.new({})
    ...
    if fetch_err
      ...
      response = UserAgentLookupResponse.new({ "err" => ctx.make_error("request_no_response", "response: undefined") })
    elsif fetched.is_a?(Hash)
      response = UserAgentLookupResponse.new(fetched)
    else
