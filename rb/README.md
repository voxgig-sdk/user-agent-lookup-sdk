# UserAgentLookup Ruby SDK



The Ruby SDK for the UserAgentLookup API — an entity-oriented client using idiomatic Ruby conventions.

The SDK exposes the API as capitalised, semantic **Entities** — for example `client.UserAgent` — with named operations (`load`) instead of raw URL paths and query strings. Working with resources and verbs keeps call sites self-describing and reduces cognitive load.
...
```ruby
client = UserAgentLookupSDK.test(testopts, sdkopts)
```
...
```
rb/
├── UserAgentLookup_sdk.rb       -- Main SDK module
...
The main module (`UserAgentLookup_sdk`) exports the SDK class
