...
class UserAgentEntity
  ...
  def make
    opts = @_entopts.dup
    UserAgentEntity.new(@_client, opts)
  end

  def data_set(args)
    if args
      @_data = UserAgentLookupHelpers.to_map(VoxgigStruct.clone(args)) || {}
      @_utility.feature_hook.call(@_entctx, "SetData")
    end
  end

  # @return [UserAgent, Hash] the current UserAgent data
  def data_get
  ...
  def match_set(args)
    if args
      @_match = UserAgentLookupHelpers.to_map(VoxgigStruct.clone(args)) || {}
      @_utility.feature_hook.call(@_entctx, "SetMatch")
    end
  end

  # @return [Hash] the current match filter (any subset of UserAgent fields)
  def match_get
  ...
  #
  # @param reqmatch [UserAgentLoadMatch, Hash, nil] match criteria (id/query fields);
  #   optional — an entity with no id-like key loads with no match (nil is treated
  #   as an empty match, so client.UserAgent.load works with no args).
  # @param ctrl [Object, nil] optional per-call control
  # @return [UserAgent, Hash] the loaded UserAgent; raises UserAgentLookupError on failure
    ...
    _run_op(ctx) do
      if ctx.result
        ...
        if ctx.result.resdata
          @_data = UserAgentLookupHelpers.to_map(VoxgigStruct.clone(ctx.result.resdata)) || {}
        end
      end
    end
  end
