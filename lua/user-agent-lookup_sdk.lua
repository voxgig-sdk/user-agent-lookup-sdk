...
require("user-agent-lookup_types")
...
local UserAgentLookupSDK = {}
UserAgentLookupSDK.__index = UserAgentLookupSDK
...
UserAgentLookupSDK._make_feature = _make_feature


function UserAgentLookupSDK.new(options)
  local self = setmetatable({}, UserAgentLookupSDK)
  self.mode = "live"
  self.features = {}
  self.options = nil
...
function UserAgentLookupSDK:options_map()
...
function UserAgentLookupSDK:get_utility()
  return Utility.copy(self._utility)
end


function UserAgentLookupSDK:get_root_ctx()
  return self._rootctx
end


function UserAgentLookupSDK:prepare(fetchargs)
...
function UserAgentLookupSDK:direct(fetchargs)
...
function UserAgentLookupSDK:_op_allowed(op)
  local allow = vs.getpath(self.options, "allow.op")
  return type(allow) == "string" and allow:find(op, 1, true) ~= nil
end


function UserAgentLookupSDK:_op_denied(op)
  local allow = vs.getpath(self.options, "allow.op")
  if type(allow) ~= "string" then allow = "" end
  return {
    ok = false,
    err = "UserAgentLookupSDK: " .. op .. ": operation not allowed by" ..
...
-- by passing it.
function UserAgentLookupSDK:_raw_request(fetchargs)
  local utility = self._utility
...
function UserAgentLookupSDK:graphql(query, variables, ctrl)
  ...
  if type(errors) == "table" and 0 < #errors then
    ...
    res.ok = false
    res.err = "UserAgentLookupSDK: graphql: " .. msg
    res.graphql = errors
  end
...
function UserAgentLookupSDK.test(testopts, sdkopts)
  sdkopts = sdkopts or {}
  ...
  local sdk = UserAgentLookupSDK.new(sdkopts)
  sdk.mode = "test"
