...
GitHub release tag (`lua/vX.Y.Z`, see [Releases](https://github.com/voxgig-sdk/user-agent-lookup-sdk/releases)),
...
```lua
local useragent, err = client:UserAgent():load({ ua = "example_ua" })
...
```lua
local useragent, err = client:UserAgent():load({ ua = "example" })
if err then error(err) end
```
...
local result, err = client:UserAgent():load({ ua = "example" })
...
Create an instance: `local user_agent = client:UserAgent(nil)`
...
```lua
local user_agent, err = client:UserAgent():load({ ua = "ua" })
```
...
```
lua/
├── user-agent-lookup_sdk.lua    -- Main SDK module
...
The main module (`user-agent-lookup_sdk`) exports the SDK constructor
