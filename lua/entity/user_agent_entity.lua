...
local UserAgentEntity = {}
UserAgentEntity.__index = UserAgentEntity


function UserAgentEntity.new(client, entopts)
  entopts = entopts or {}
  ...
  local self = setmetatable({}, UserAgentEntity)
...
function UserAgentEntity:get_name()
  return self._name
end


function UserAgentEntity:make()
  local opts = {}
  for k, v in pairs(self._entopts) do
    opts[k] = v
  end
  return UserAgentEntity.new(self._client, opts)
end
...
function UserAgentEntity:mark_deleted()
  self._deleted = true
end


function UserAgentEntity:deleted()
  return true == self._deleted
end


function UserAgentEntity:data_set(args)
  if args ~= nil then
...
function UserAgentEntity:data_get()
  self._utility.feature_hook(self._entctx, "GetData")
  return vs.clone(self._data)
end


function UserAgentEntity:match_set(args)
  if args ~= nil then
...
function UserAgentEntity:match_get()
...
function UserAgentEntity:stream(action, args, callopts)
...
function UserAgentEntity:_run_op(ctx, post_done)
  local utility = self._utility
