...
// UserAgentLookup SDK retry feature
...
class UserAgentLookupRetryFeature extends UserAgentLookupBaseFeature
{
    ...
    public function init(UserAgentLookupContext $ctx, array $options): void
    {
        ...
        $utility->fetcher = function (UserAgentLookupContext $ctx2, string $url, array $fetchdef) use ($inner): array {
            return $this->_with_retry($ctx2, $url, $fetchdef, $inner);
        };
    }

    private function _with_retry(UserAgentLookupContext $ctx, string $url, array $fetchdef, callable $inner): array
    {
        $opts = $this->options ?? [];
    ...
    private function _track(UserAgentLookupContext $_ctx, int $attempt, mixed $res, mixed $err, float $wait): void
    {
        $client = $this->client;
