...
// UserAgentLookup SDK UserAgent entity
...
class UserAgentEntity
{
    ...
    {
        $opts = $this->_entopts;
        return new UserAgentEntity($this->_client, $opts);
    }

    /**
     * @param UserAgent|array $args UserAgent data (assoc-array) to store.
     */
    public function data_set($args): void
    {
        if ($args) {
            $this->_data = UserAgentLookupHelpers::to_map(Struct::clone($args)) ?? [];
            ($this->_utility->feature_hook)($this->_entctx, "SetData");
        }
    }

    /**
     * @return UserAgent|array The current UserAgent data as an assoc-array.
     */
    ...
    /**
     * @param array $args Match filter (any subset of UserAgent fields).
     */
    public function match_set($args): void
    {
        if ($args) {
            $this->_match = UserAgentLookupHelpers::to_map(Struct::clone($args)) ?? [];
            ($this->_utility->feature_hook)($this->_entctx, "SetMatch");
        }
    }

    /**
     * @return array The current match filter (any subset of UserAgent fields).
     */
    ...
    /**
     * Load a single UserAgent.
     *
     * @param UserAgentLoadMatch|array|null $reqmatch Match criteria (id/query
     *   fields) as an assoc-array; a typed UserAgentLoadMatch names the shape.
     * @param mixed $ctrl Optional per-call control overrides.
     * @return UserAgent|array The loaded UserAgent as an assoc-array at the
     *   SDK boundary; throws UserAgentLookupError on failure (item-5 convention).
     */
        ...
        return $this->_run_op($ctx, function () use ($ctx) {
            if ($ctx->result) {
                ...
                if ($ctx->result->resdata) {
                    $this->_data = UserAgentLookupHelpers::to_map(Struct::clone($ctx->result->resdata)) ?? [];
                }
            }
        });
    }
